...
class JuiceboxFormatter implements JuiceboxFormatterInterface, TrustedCallbackInterface {
  ...
  public static function preRenderFieldsets(array $form) {
    foreach (Element::children($form) as $key) {
      ...
      // property to any element that belongs in a fieldset. If this
      // form element has that property, move it into its fieldset.
  ...
  public function confBaseStylePresets($allow_multisize = TRUE) {
    ...
    if ($allow_multisize && array_key_exists('disallowed_conf', $library) && !in_array('juicebox_multisize_image_style', $library['disallowed_conf'])) {
