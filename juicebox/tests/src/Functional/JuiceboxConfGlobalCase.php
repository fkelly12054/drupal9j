...
class JuiceboxConfGlobalCase extends JuiceboxCaseTestBase {
  ...
  public function setUp(): void {
    ...
    $this->initNode();
    // Activate the field formatter for our new node instance.
  ...
  public function testGlobalConf() {
    ...
    $this->drupalLogout();
    $this->drupalGet('juicebox/xml/field/node/' . $node->id() . '/' . $this->instFieldName . '/full');
    // Check the XML now returns an 'Access-Control-Allow-Origin' header
    // for CORS support.
  ...
  public function voidtestGlobalTrans() {
    ...
    $this->assertSession('The strings have been saved. Languagelist translation saved.');
    // Now check the resulting XML again as an anon user.
    $this->drupalLogout();
    $this->drupalGet('juicebox/xml/field/node/' . $node->id() . '/' . $this->instFieldName . '/full');
    // Check that the languagelist configuration option was both included and
    // translated in the XML.
    $this->assertSession('languagelist="Translated|Lang|List. Translated languagelist value found in XML."');
  }
  ...
  public function testGlobalMultisize() {
    ...
    $this->drupalGet('admin/config/media/juicebox');
    $this->submitForm($edit, 'Save configuration');
    $this->assertSession('The Juicebox configuration options have been saved. Custom global options saved.');
