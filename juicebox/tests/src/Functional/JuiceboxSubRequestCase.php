...
class JuiceboxSubRequestCase extends JuiceboxCaseTestBase {
  ...
  // fields won't work if the field configuration is created programmatically as
  ...
  // will have broken handlers). This much have something to do with the config
  // schema or something (programmatically created entities are not yet
  // recognized when a view config that depends on them in loaded)? I have
  // resorted to loading the field conf within a helper module
  // (juicebox_mimic_article) as it seems to get around this.
  // That module just mimics an "article" type so
   ...
   * @var string
   */
  protected string $instFieldName = 'field_image';
  ...
  public function testSubRequestDependent() {
    ...
    // that with an actual "&" char in the pattern itself.
